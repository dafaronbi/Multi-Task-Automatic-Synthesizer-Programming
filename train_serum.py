...
def main():
    ...
    train_index = np.where(train_synth ==  "serum")
    valid_index = np.where(valid_synth ==  "serum")
    test_index = np.where(test_synth ==  "serum")

    train_spec_data = train_spec_data[train_index]
    train_params = train_serum_params[train_index]
    ...
    if not os.path.isdir("/vast/df2322/asp_data/saved_models/vst_serum"):
        os.makedirs("/vast/df2322/asp_data/saved_models/vst_serum")

    # Include the epoch in the file name (uses `str.format`)
    checkpoint_path = "/vast/df2322/asp_data/saved_models/vst_serum/cp-{epoch:04d}.ckpt"
    ...
    #create model
    m = model.vae_serum(l_dim, i_dim, train_serum_params.shape[-1], train_diva_params.shape[-1], train_tyrell_params.shape[-1], model.optimizer, warmup_it)
