...
def multi_metrics():
    ...
    checkpoint_path = "saved_models/vst_single_nomask/cp-{epoch:04d}.ckpt"
    ...
    #create model
    m = model.vae_single_no_mask(64, i_dim, test_params_single.shape[-1], model.optimizer, warmup_it)

    #load stored weights
    m.load_weights(latest)

    file_name = "single_nomask_metrics.csv"
    ...
    while i < len(test_serum_params):
        ...
        if test_synth[i] == "serum":
            ...
            serum_p = one_hot.predict(np.squeeze(params)[:480],one_hot.serum_oh)
            p_p = serum_p
        ...
        if test_synth[i] == "diva":
            ...
            diva_p = one_hot.predict(np.squeeze(params)[:759],one_hot.diva_oh)
            p_p = diva_p
        ...
        if test_synth[i] == "tyrell":
            ...
            tyrell_p = one_hot.predict(np.squeeze(params)[:327],one_hot.tyrell_oh)
            p_p = tyrell_p
