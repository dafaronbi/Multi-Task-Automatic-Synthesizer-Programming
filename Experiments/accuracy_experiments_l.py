...
def multi_metrics():
    ...
    checkpoint_path = "saved_models/vst_multi_" + str(l_dim) +"/cp-{epoch:04d}.ckpt"
    ...
    #create model
    m = model.vae_multi(l_dim, i_dim, test_serum_params.shape[-1], test_diva_params.shape[-1], test_tyrell_params.shape[-1], model.optimizer, warmup_it)

    #load stored weights
    m.load_weights(latest)

    file_name = "multi_metrics_" + str(l_dim) +".csv"
