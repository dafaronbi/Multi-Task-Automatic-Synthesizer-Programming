...
def multi_metrics():
    ...
    #create model
    m = model.vae_single(64, i_dim, test_params_single.shape[-1], model.optimizer, warmup_it)

    #load stored weights
    m.load_weights(latest)

    file_name = "single_metrics.csv"
    ...
    while i < len(test_serum_params):
        ...
        spec,params = m.predict([test_spec_data[[i]], test_masks_single[[i]]])
